...
# imports.
from fil3s.classes.env import Environment
from fil3s.classes.files import *
from fil3s.classes.response import Response
from fil3s.classes.utils import copycats
from fil3s.classes import utils, objects
import platform 
...
class __Defaults__(object):
	def __init__(self):
		...
		self.options = objects.Object({
			"log_level": int(copycats.get_argument("--log-level", required=False, default=Environment.get_integer("LOG_LEVEL", default=0))),
			"cli": Environment.get_boolean("CLI", default=False),
			"json": copycats.arguments_present(["-j", "--json"]),
			"interactive": not copycats.arguments_present(["--non-interactive"]) and Environment.get("INTERACTIVE", format=bool, default=False) == True,
			"forced": copycats.arguments_present(["--forced", "-f", "--force"], default=Environment.get("FORCED", default=False, format=bool)),
			"quiet": copycats.arguments_present(["--quiet", "-q", "-s", "--silent"], default=Environment.get("QUIET", default=False, format=bool)),
		})
	...
	def alias(self,
		...
		l_alias = copycats.get_argument("--create-alias", required=False)
		present = "--create-alias" in sys.argv and l_alias == alias
		base = f"/usr/local/bin"
		if not Files.exists(base):
			base = f"/usr/bin/"
		path = f"{base}/{alias}"
		if ((copycats.argument_present("--force") or copycats.argument_present("--forced") or overwrite) and present) or (present or not Files.exists(path)):
	...
	def log_level(self, default=0):
		return int(copycats.get_argument("--log-level", required=False, default=Environment.get_integer("LOG_LEVEL", default=default)))
